...
在网格化城市路网中,起点 $S$ 与卸货点 $T$ 物理分离,车辆需在容量约束下规划多次行程访问全部收集点,使总行驶距离最短。本文将其形式化为带专属卸货点的容量受限多行程旅行商问题,并提出由内层子集旅行商动态规划与外层划分动态规划组成的两层精确求解结构;在划分 DP 的内层子集枚举上,引入基于轴元素 (pivot) 的分治构造,使每个候选划分仅被枚举一次,理论枚举量减半。本文进一步将该框架扩展至双车场景,通过 $2^n$ 二划分枚举确定收集点分配;另实现最近邻贪心作为启发式对照,并以 C++17 后端 + PyQt6 前端的双层架构落地,前后端经行式文本协议解耦。证明方面,给出划分 DP 最优性 (引理 1) 与分治枚举对标准枚举等价性 (定理 1) 的形式化结论,后者基于"后续行程顺序可交换"性质 (引理 2) 严格论证。实证方面,设计 260 例独立暴力对拍,确认动态规划解与暴力解完全一致;在三组规模递增 ($n in {4,6,8}$) 的样例上,分治变体相对标准枚举节省约 $17%$ 端到端运行时间,双车协同相对单车节省 $3%$–$12%$ 总距离;构造一个 $3 times 10$ 对抗实例,定量验证最近邻贪心可严格次优(差距约 $15%$),并在另两组实测中观察到 $7%$–$22%$ 的次优差。
...
车辆路径问题 (Vehicle Routing Problem, VRP) 自 Dantzig 与 Ramser (1959) 提出以来,衍生出若干面向具体应用场景的变体。本文研究其中一类与城市垃圾收运高度匹配的变体:车队从固定停车场 $S$ 出发,沿离散化的城市路网访问若干带重量的收集点,载重达到上限后必须前往与 $S$ 物理分离的处理厂 $T$ 卸货,卸货后可继续作业,直至所有收集点恰被访问一次。优化目标是行程总距离最短。
...
本文围绕该问题做了如下工作:在内层子集旅行商 DP 与外层划分 DP 构成的两层求解框架基础上 (@sec:algo),引入基于轴元素 (pivot) 的分治法子集枚举,使每个候选划分仅被枚举一次,理论枚举量减半 (@sec:divconq);将该框架扩展至双车场景,以 $2^n$ 二划分枚举确定收集点分配 (@sec:dual);形式化证明划分 DP 的最优性 (引理 1) 与分治枚举对标准枚举的等价性 (定理 1);通过 260 例随机暴力对拍独立检验 DP 与暴力解的一致性,并构造一个 $3 times 10$ 对抗实例,定量演示最近邻贪心可能严格次优 (@sec:exp);此外提供基于 PyQt6 的图形化前端,支持交互式编辑、五种算法的即时切换与多车并发动画 (@sec:gui)。
...
#emph[情形 B]: $"pivot" in.not Q^*$,即 $"pivot" in "mask" without Q^*$。由引理 1,$G("mask" without Q^*)$ 对应一个 later-trip 划分 $cal(L)' = (R_1, ..., R_l)$。因 $"pivot" in "mask" without Q^*$,必存在某 $R_(j_0)$ 含 $"pivot"$。由引理 2,把 $R_(j_0)$ 提到 mask 层的首条 later-trip,其余 trips $cal(L)'' = cal(L)' without {R_(j_0)} union {Q^*}$ 作为 $"mask" without R_(j_0)$ 的 later-trip 划分,总代价完全不变。

新分解 $cal(M) = (R_(j_0), cal(L)'')$ 满足:首条 trip $R_(j_0)$ 含 pivot,故 $cal(M)$ 在分治枚举的候选中出现。因此
$
G^("dc")("mask") &<= "laterCost"(R_(j_0)) + G^("dc")("mask" without R_(j_0)) \
                 &= "laterCost"(R_(j_0)) + G^("std")("mask" without R_(j_0)) quad ("归纳") \
                 &<= "laterCost"(R_(j_0)) + sum_(j != j_0) "laterCost"(R_j) + "laterCost"(Q^*) - "laterCost"(R_(j_0)) \
                 &= sum_j "laterCost"(R_j) + "laterCost"(Q^*) - "laterCost"(R_(j_0)).
$
化简(由引理 2 重排不改代价)得 $G^("dc")("mask") <= "laterCost"(Q^*) + G^("std")("mask" without Q^*) = G^("std")("mask")$。

合并两情形,$G^("dc") <= G^("std")$。结合反向,$G^("dc") = G^("std")$。
]
