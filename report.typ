...
#set page(
  ...
  numbering: "i"   // 前置部分用罗马数字, 进入正文前切换为阿拉伯数字
)
#set text(
...
#align(center)[
  ...
  #grid(
    ...
    text(weight: "bold")[班　　级], [人工智能 2024 级],
    ...
    text(weight: "bold")[学年学期], [2025–2026 学年 第二学期],
...
在网格化城市路网中, 起点 $S$ 与卸货点 $T$ 物理分离, 车辆需在容量约束下规划多次行程访问全部收集点, 使总行驶距离最短。本文将其形式化为带专属卸货点的容量受限多行程旅行商问题, 提出由内层子集旅行商动态规划与外层划分动态规划组成的两层精确求解结构, 并在划分 DP 的子集枚举上引入基于轴元素 (pivot) 的对称性消除, 在常数意义上把候选规模减半。框架进一步扩展至双车场景, 通过子集划分枚举 + `singleCost[mask]` 表复用实现高效求解;另实现最近邻贪心作为启发式对照, 整体以 C++17 后端 + PyQt6 前端的双层架构落地, 经行式文本协议解耦。理论方面给出划分 DP 最优性、pivot 枚举对标准枚举等价性 (基于"后续行程顺序可交换"性质) 的形式化证明。实证方面通过固定种子的随机暴力对拍验证 DP 实现与暴力枚举的完全一致, 并在跨规模、跨障碍密度的密集基准上比较五种算法的总距离、运行时间与双车负载均衡。结果表明: 两种 DP 实现的输出在所有样例上严格相同; 最近邻贪心存在结构性次优, 在对抗实例上代价显著高于 DP; 双车协同相对单车稳定降低总距离, 经 `singleCost` 优化后运行时间与单车持平。
...
随着城市规模与人口密度持续上升, 生活垃圾日产量逐年增加, 收运环节的运行成本已成为环卫部门的主要支出之一; 同时, 新能源垃圾收运车辆的续航受限与碳排放配额收紧, 也都要求在路径层面尽量压缩冗余里程。在这一现实背景下, 对网格化路网下的多行程收运路径进行精确建模与最优化求解, 既能为车辆调度系统提供基线策略, 也能为人力/能耗/碳排预算的精算提供可比对的参考解。
...
== 输入合法性与不可行判定 <sec:feas>

任务书明确要求"对非法输入报错并允许用户重新输入"。本系统将这一职责集中到独立模块 `feasibility.cpp::check_feasibility()`, 在算法分派之前一次性完成全部静态检查; 通过检查的输入才会进入距离矩阵预计算与算法求解, 任何一项失败均立即终止求解并输出 `STATUS infeasible / REASON <reason>` 行式协议条目, 由 GUI 前端解析后弹出错误对话框, 用户可立即在网格上修改并重试。

具体的 REASON 类别如 @tab:feas 所列, 共 5 类、覆盖任务书规定的所有约束。这些 REASON 文本是稳定的契约: 自动化测试 `tests/verify_sample_library.py` 中 8 个不可行实例的预期分类 (见 `data/sample_library/`) 即以此为依据, 在 CI 中持续校验。
...
#figure(
  table(
    ...
    [*类别*], [*触发条件*], [*REASON 文本*],
    [关键点落在障碍/越界], [任一 $S$/$T$/$P_i$ 坐标越出网格或落在 `#` 格], [`<name> 落在障碍或越界 (r,c)`],
    [$S = T$ 重合], [停车场与处理厂坐标完全相同], [`停车场 S 与处理厂 T 不能重合`],
    [关键点坐标重复], [任两关键点 (含 $S,T$ 及 $P_i$) 坐标相同], [`关键点坐标重复: P<i>`],
    [重量越界], [存在 $w_i in.not [1, 3]$], [`收集点 P<i> 的重量不在 1..3 范围内`],
    [容量约束矛盾], [$W_max < max_i w_i$ 或 $W_max >= sum_i w_i$], [`W_max 小于单个最大收集点重量 (...)` / `总重量 ≤ W_max, 一次行程即可完成`],
    [BFS 不可达], [任一关键点与 $S$ 的 BFS 距离为 $-1$], [`关键点 <name> 与停车场 S 不可达`],
  ),
  caption: [`feasibility` 模块识别的 6 类不可行情形与对应 REASON 文本契约。],
) <tab:feas>

以上检查中, 前 5 类是 $O(K)$ 的静态比较, 第 6 类(可达性)由从 $S$ 出发的一次 BFS 在 $O(M L)$ 内完成; 整个 `check_feasibility` 总开销不超过预计算阶段中任意一次 BFS, 不构成性能瓶颈。
...
== 基于 pivot 的子集枚举规范化 (分治法实现) <sec:divconq>

任务书加分项 (3) 要求"用分治法对子集枚举进行优化"。本节给出的优化在结构上等价于按 pivot 元素的二分支分治: 对当前 mask, 取最低位元素作为 pivot, 把所有候选子集 $Q subset.eq "mask"$ 分为两支 —— $"pivot" in Q$ 与 $"pivot" in.not Q$; 只对前者显式枚举, 后者由含 pivot 的递归子问题 $G("mask" without Q')$ 自然覆盖。这与归并排序"按位置二分、递归求解、合并"的分治范式同构, 故本文等价地称之为"基于 pivot 的子集枚举规范化"。
...
实验环境的硬件 / 软件配置如 @tab:env 所列。所有实验在该单一物理机上完成, 测量期间无其他高负载任务并发。
...
#figure(
  table(
    ...
    [架构], [AMD64],
    [C++ 编译器], [g++ 15.2.0 (MSYS2 mingw64), `-std=c++17 -O2 -Wall -Wextra`],
    [Python 解释器], [Python 3.13 (Anaconda conda env `pytorch`)],
    [GUI 框架], [PyQt6 6.x],
    [报告排版], [Typst 0.14.2],
    [测试种子文件], [`tests/seeds.txt` (60 个素数, 范围 $[1, 280]$)],
  ),
  caption: [实验环境配置。],
) <tab:env>
...
实验所用的随机实例统一由 `tests/random_case_generator.py` 生成, 调用接口为 `generate(seed, n, ratio)`, 返回包含网格、关键点、重量、$W_max$ 的实例 dict 或 `None` (生成失败时)。生成流程为:

+ 由 `seed` 确定独立的伪随机数发生器, 保证可复现;
+ 网格尺寸 $M, L$ 在 $[8, 12]$ 间随机选取, $S = (0, 0)$, $T = (M-1, L-1)$ 固定位于对角两端;
+ 按障碍密度 $rho$ 随机翻转网格的 `.`/`#`, 同时保证 $S, T$ 与即将放置的收集点格保持 `.`;
+ 随机采样 $n$ 个收集点 $P_i$ (互不重合, 不与 $S, T$ 重合), 各自分配 $w_i in {1, 2, 3}$;
+ 取 $W_max = max(max_i w_i, floor(sum_i w_i / 2))$ 以保证 $max_i w_i <= W_max < sum_i w_i$ 同时强制多行程结构;
+ 最后调用 `feasibility::check_feasibility` 验证可达性, 失败则返回 `None`, 由调用方跳过 (SKIP)。

该生成器同时被 §6.4 暴力对拍 (规模较小, $rho in {0, 0.20}$) 和 §6.8 密集基准 (规模 $n in [3, 8]$, $rho in {0, 0.10, 0.20, 0.30}$) 复用, 保证两个实验同源, 后者只是更密集地扫描了 $(n, rho)$ 二维参数空间。
...
#text(weight: "bold")[贪心在随机样本上的成功率与平均次优差.] 在 §6.4 同源的随机样本集合上 (62 种子 × $n in {3,4,5}$ × $rho in {0, 0.20}$, 共 372 例, 其中 338 例可行、34 例被求解器与生成器同步识别为不可行) 用 `greedy` 重跑全部 338 个可行实例: 贪心 *100% 完成访问* (无超载、无漏点、status 全部为 `ok`), 这是因为算法实现层 `load + w_i <= W_max` 与 BFS 距离 $>= 0$ 的硬过滤保证了每一步都合法。但在最优性上, 共 *105 例 (~31%) 贪心解严格劣于 DP 解, 平均超出 14.5%*, 最坏一例 (seed=131, $n=5, rho=0.20$) DP 解 30 而贪心 66, 多出 120%。该结果表明: 最近邻贪心作为快速基线工程上稳定可行, 但作为优化方法在结构性输入上代价不可忽略。
...
#figure(
  table(
    ...
    [*样例*], [*总距离*], [*$"dist"_1$*], [*$"dist"_2$*], [*$"load"_1$*], [*$"load"_2$*], [*距离均衡度*], [*载重均衡度*],
    [`small`],  [30], [14], [16], [1], [6],  [93.3%], [28.6%],
    [`medium`], [62], [26], [36], [4], [6],  [83.9%], [80.0%],
    [`large`],  [84], [28], [56], [3], [10], [66.7%], [46.2%],
  ),
  caption: [双车 DP 在三组样例上的距离与载重对比 (载重 $= sum_("trip" in "vehicle") "load"_("trip")$, 均衡度 $= 1 - |Delta| / ("sum")$)。],
) <tab:balance>

观察可见两个均衡度均随规模递减: 小样例两车距离接近 (1:1.14), 但载重严重失衡 (车 1 仅 1 单位载重而车 2 高达 6); 大样例距离 1:2 且载重 1:3.3, 车 1 仅承担小部分收集任务。这一现象的成因是 `multi_dp` 的目标函数只显式优化两车总距离: 当一辆车走"主干"路径已能高效串联大部分收集点时, 把剩余几个偏远点单独划给另一辆车跑短程, 反而比强行均分更省总距离。换言之, 均衡是优化的副产品而非目标。如需强约束负载均衡(例如人员工时公平或车辆轮换), 应在目标函数中引入 $|"dist"_1 - "dist"_2|$、$max("dist"_1, "dist"_2)$ 或 $|"load"_1 - "load"_2|$ 等正则项, 这是本文未涵盖的扩展方向。

#text(weight: "bold")[与单车的对比.] 同样三组样例下, 单车 `dp` 的总距离为 34/64/92, 即双车协同在 `small`/`medium`/`large` 上分别相对单车节省 $11.8%$/$3.1%$/$8.7%$。值得注意的是, 单车解的载重序列分别为 $(3, 3, 1)$ / $(4, 4, 2)$ / $(5, 5, 3)$, 与双车解的载重总和 (7 / 10 / 13) 完全一致 — 因为收集任务的总重量是输入决定的不变量, 双车只是在车辆维度上重新分配, 不影响 $sum_i w_i$。
...
@fig:gui 展示前端窗口在静态编辑与动画运行两种状态下的截图。前端覆盖了任务书加分项 (2) 全部 6 项功能:

+ *网格、障碍与关键点的可视化*: 基于 `QGraphicsView` 渲染 `M times L` 网格, 障碍格为深灰矩形, 关键点 ($S$ 绿、$T$ 红、$P_i$ 蓝) 以圆角卡片样式叠加, 带阴影;
+ *鼠标交互式编辑*: 左侧编辑模式按钮 (障碍 / S / T / P / 擦除) 互斥选择, 在网格上单击放置、右键擦除; 收集点重量由旁侧 SpinBox 设定;
+ *算法即时切换*: 顶部下拉框可在 `dp / dp_dc / greedy / multi_dp / multi_greedy` 五种算法间一键切换;
+ *多色路径动画*: 不同行程使用不同颜色 (8 色调色板), 基于 `QVariantAnimation` + `InOutQuad` 缓动曲线, 步长 $180#h(1pt) "ms"$ 每格, 多车场景下各车独立步进;
+ *实时载重与已行驶距离*: 右栏顶部 KPI 卡片条显示聚合指标 (总距离 / 耗时 / 车辆数 / 总行程数); 下方"实时状态"面板逐车一行, 动画过程中按格刷新当前 trip 序号、当前载重与已行驶距离 (相对动画起点的累计步数), 完成时整行背景由浅蓝切换为浅绿; 配合右上角状态徽章 ("等待 → 运行中 → 完成 → 错误", 颜色编码灰 / 黄 / 绿 / 红), 用户在任何时刻都能从颜色与数字判断求解器的当前阶段;
+ *最终输出与行程详情*: 动画结束后, KPI 卡片定格在最终聚合值; 底部"行程详情"等宽字体区域列出每辆车每条 trip 的载重、距离与访问点序列, 便于截图与逐项校对。

实时刷新通过 `animator.py` 暴露的 `status_callback(vi, info)` 钩子实现: 在 ① 构造完成时 (初始态)、② 每完成一格、③ trip 切换、④ 全部完成四个时机被触发, `info` 字段包括 `trip_idx / num_trips / load / distance / done`, UI 端解析后更新对应车辆行的文字与样式, 无需轮询。
...
#figure(
  ...
  caption: [PyQt6 图形化前端。左:载入 `sample_medium` 后运行 `dp` 的主界面 (左侧编辑工具栏 / 中央网格 / 右栏 KPI + 实时状态 + 行程详情);右:动画运行中,小车沿规划路径平滑移动,右栏实时状态行随每格刷新载重与已行驶距离。],
) <fig:gui>

// ===================== 7 结论 =====================
= 结论

本文在网格化城市路网下研究了带专属卸货点的容量受限多行程旅行商问题。引理 1 与 定理 1 给出了精确求解的最优性与分治枚举对标准枚举等价性的形式化证明, 后者通过"后续行程顺序可交换" (引理 2) 的辅助性质完成。在三组规模递增的网格实例与 720 例独立暴力对拍上, 两种 DP 实现的输出值与暴力解完全一致 (658 PASS / 0 MISMATCH / 62 SKIP, SKIP 为生成器同步识别的不可行实例)。在 1940 数据点的密集基准上, `dp` 与 `dp_dc` 输出值在所有 $n$ 上严格相同, 而 pivot 枚举的端到端实际加速仅为 $0$–$1%$ 量级 —— 理论上其子集枚举量减半, 但子集枚举仅占完整 DP 流程的一小部分常数。双车协同相对单车在 $n in [3, 8]$ 上稳定降低总距离 $4.6%$–$5.0%$; 经 `singleCost[mask]` 表复用 + 对称性消除的 P6 优化后, 双车 DP 与单车 DP 运行时间持平 (优化前约 $460 times$ 慢)。最近邻贪心在结构对抗实例上代价比 DP 高 $15.4%$; 在 338 例随机可行样本上贪心成功率 $100%$, 但其中 $31%$ 严格次优, 平均超出 DP 解 $14.5%$, 最坏一例超出 $120%$。
